...
import { Seat, PenPath, TextElement, ShapeMode, SeatMapType } from "../types";
...
interface SeatState {
  ...
  // NEW: Seat count logic (exact replica of your promptForSeatCount)
...
export const useSeatStore = create<SeatState>((set, get) => ({
  ...
  // UPDATED: Seat selection with count validation (exact replica of your logic)
  ...
  // NEW: Count available seats (exact replica from script.js)
  countAvailableSeats: () => {
    const state = get();
    return state.currentSeats.filter(seat => !seat.occupied).length;
  },

  // NEW: Prompt for seat count (exact replica of your vanilla function)
  promptForSeatCount: () => {
    const state = get();
    const availableSeats = state.countAvailableSeats();

    if (availableSeats === 0) {
      alert("No seats are available for booking.");
      return false;
    }
  ...
  // Pan/Zoom functions (exact replica from your script.ts)
  ...
  updatePan: (isDesigner, clientX, clientY) =>
    set((state) => {
      if (isDesigner && state.isDesignerPanning) {
        ...
        return {
          designerPanX: state.designerPanX - dx,
          designerPanY: state.designerPanY - dy,
          panStart: { x: clientX, y: clientY },
        };
        ...
        return {
          userPanX: state.userPanX - dx,
          userPanY: state.userPanY - dy,
          panStart: { x: clientX, y: clientY },
        };
      }
      return state;
